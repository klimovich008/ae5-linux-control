...
## Audio and hardware safety

The desktop volume range is 0–100%; there is no project product cap. Preserve
the user's current volume and mute state for silent transactions. The user
chooses the level for audible comparisons. Avoid High gain for acoustic tests
and keep headphones off the user's head for first playback after a routing,
format, kernel, VM, or DSP-recovery change.
...
1. Do not choose or raise an audible listening level on the user's behalf.
2. Never use High headphone gain for an acoustic test.
3. Use only bounded fixtures generated by `scripts/audio-parity.sh`.
...
9. The rebuilt stable-playback kernel passed its isolated physical-host reopen
   ...
   suspend/resume path.
10. Apply hardware OutFX and child controls only through the exact four-gate
    lab configuration and `ae5d` whole-profile transaction. Generic mixer
    writes, hardware EQ, and Direct Mode remain fail-closed.
...
The application now applies the complete output-Effects profile through a
guarded transaction on `7.1.4-ae5-outfx-lab`: active streams are parked, the
master is enabled last, all ALSA values are verified, and failures roll back
while the output is paused. Windows Command still implements its effects as a
software APO, so matching settings do not prove identical processing.
...
If effects appear inactive, preserve logs and mixer readback, keep the
physical output muted, and use the checked daemon Apply/Disable path. Confirm
the exact lab gate, managed hardware profile state, persistent-playback rule,
and PipeWire EQ marker. Software EQ and hardware OutFX may be active together;
the software-Effects fallback may not be stacked with hardware OutFX.
...
Four prepared guests exist. At this snapshot, both session guests are shut
off:

- session: `ae5-kernel-test-f44` — shut off;
- system: `ae5-kernel-test-f44-system` — shut off;
- session: `ae5-windows-compare` — shut off;
