...
which setting must change. The shared backend applies the same guard to CLI and
profile writes, so those constraints cannot be bypassed outside the GUI. The
app listens for native ALSA mixer events, so
